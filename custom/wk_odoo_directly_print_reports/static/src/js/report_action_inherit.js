...
function makeActionManager(env) {
    ...
    function _makeReportUrls(action, type){
        let url = `/report/${type}/${action.report_name}`;
        const actionContext = action.context || {};
        if (action.data && JSON.stringify(action.data) !== "{}") {
            // build a query string with `action.data` (it's the place where reports
    ...
    function get_model_popup(cause, list=[]){
        switch(cause){
            case 'error':
                ...
                '<div class="modal-dialog modal-sm">'+
                    '<div class="modal-content">'+
                        ...
                        `<button type="button" class="close" data-dismiss="modal" onclick="document.getElementById('myModalpopup').remove()">&times;</button>`+
    ...
    async function _executeReportAction(action, options) {
        let activeIds = action.context.active_ids;

        const activeIdsContext = {
            active_id: activeIds[0],
            active_ids: activeIds,
            active_model: action.context.active_model,
        };
        const context = new Context(action.context, activeIdsContext, action.data).eval();

        var data = await rpc.query({
            model: 'ir.actions.report',
            method: 'search_read',
            args: [[['report_name', '=', action.report_name]]],
        })

        if (data[0]['report_user_action'] == 'send_to_printer'){
            data[0].context = context;
            var reportUrls = _makeReportUrls(data[0], data[0]['report_type']);
            ...
            return qz.websocket.connect().then(function(){
                ...
                .then(function(printer_find_opr){
                    if(printer_find_opr && printer_name){
                        ...
                        get_all_printers().then(function(get_printers_opr){
                            var model_popup = get_model_popup("listing", get_printers_opr)                  
                            $("#myModalpopup").remove();
                            ...
                            $('.print_now').on("click", function(e){
                                ...
                                var args = [reportUrls, zpl_report, context];
                                rpc.query({
                                    model:'ir.actions.report',
                                    method:'get_zpl_data',
                                    args: args})
                ...
                qz.websocket.disconnect();
                var model_popup = get_model_popup("error")
                // 
                $('body').append(model_popup);
                $("#myModalpopup").modal("show");
            });
...
patch(actionService, 'actionServiceUpdate',{
    start: function(env) {
