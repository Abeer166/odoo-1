...
class IrActionsReport(models.Model):
    ...
    printer_type = fields.Selection(string="Type", readonly=True, related="printer_id.printer_type")
    ...
    def report_routes(self, reportname, docids=None, converter=None, **data):
        ...
        if converter == 'pdf':
            pdf = report.with_context(context)._render_qweb_pdf(reportname, data=data)[0]
        elif converter == 'text':
            pdf = report.with_context(context)._render_qweb_text(reportname, data=data)[0]
        return pdf
    ...
    @api.model
    def get_zpl_data(self, qweb_url, zpl_report, ctx_data={}, printer_name=False):
        try:
            data = []
            qweb_source_parts = qweb_url.split('/')
            _logger.info("- -- - -- - - %r - -- - --url -- - -- "%qweb_source_parts)
            ...
            if report.use_template:
                ...
                _logger.info("---------Dataaaaaaaaaaa---1---%r", data)
                return data
            reportname = qweb_source_parts[-2]
            docids = qweb_source_parts[-1]
            if zpl_report:
                document = self.report_routes(reportname, docids=docids, converter="text", data=ctx_data)
                data.append(document)
                _logger.info("---------Dataaaaaaaaaaa---2---%r", data)
                return data
            document = self.report_routes(reportname, docids=docids, converter="pdf", data=ctx_data)
            _logger.info("1+++++++++++%r", type(document))
            pages = GRF.from_pdf(document, 'DEMO')
            _logger.info("2+++++++++++%r",pages)
            fd, file_name = mkstemp()
            _logger.info("3+++++++++++%r",[file_name,fd] )
            ...
            _logger.info("---------Dataaaaaaaaaaa---3---%r", data)
            return data
        except Exception as e:
            raise Warning(e)
