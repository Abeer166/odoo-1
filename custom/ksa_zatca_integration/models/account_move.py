...
class AccountMove(models.Model):
    ...
    zatca_onboarding_status = fields.Boolean(readonly=1, default=lambda self: self.env.company.sudo().zatca_onboarding_status,
                                             copy=False)
    ...
    def unlink(self):
        ...
        message = "Based on the VAT regulation, after issuing an invoice, it is prohibited to " \
                  ...
                  " modified invoice"
        if conf.is_zatca and self.state != 'draft' and self.move_type in ['out_invoice', 'out_refund']:
    ...
    def button_draft(self):
        ...
        message = "Based on the VAT regulation, after issuing an invoice, it is prohibited to " \
                  ...
                  " the supplier should issue an electronic credit/debit note linked to the original." \
                  " modified invoice"
        if conf.is_zatca and  self.state == 'posted' and self.move_type in ['out_invoice', 'out_refund'] and self.zatca_status_code != '400':
            raise exceptions.AccessDenied(message)
    ...
    def button_cancel(self):
        ...
        message = "Based on the VAT regulation, after issuing an invoice, it is prohibited to " \
                  ...
                  " the supplier should issue an electronic credit/debit note linked to the original." \
                  " modified invoice"
        if conf.is_zatca and  self.state == 'posted' and self.move_type in ['out_invoice', 'out_refund']:
            raise exceptions.AccessDenied(message)
